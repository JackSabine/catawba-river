...
assign wr_if.full  = (head == tail) && (head_phase != tail_phase);
assign rt_if.empty = (head == tail) && (head_phase == tail_phase);
assign head_ready = rob[head].ready & ~rt_if.empty;
...
assign rt_if.head_exception = rob[head].exception;

logic legal_push;
logic legal_pop;

assign legal_push = wr_if.push && !rt_if.full;
assign legal_pop  = rt_if.pop && !rt_if.empty;
...
for (i = 0; i < ROB_DEPTH; i++) begin : rob_entries
    always_ff @(posedge clk) begin
        unique0 if (wb_if.valid && (wb_if.rob_index == i)) begin
            ...
            rob[i].exception = wb_if.exception;
