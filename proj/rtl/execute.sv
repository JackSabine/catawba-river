...
module execute import catawba_types::*; #(
    ...
    input wire [XLEN-1:0] read_port_data_2,
    input wire [XLEN-1:0] immediate,
    input wire [XLEN-1:0] next_pc,

    input wire alu_use_pc,
    input wire alu_use_imm,
    input alu_operation_e alu_operation,
    input branch_alu_operation_e branch_alu_operation,

    input wire is_branch_inst,

    output wire take_branch,
    output wire [XLEN-1:0] alu_output,
    output wire [XLEN-1:0] read_port_data_for_memory
);

wire [XLEN-1:0] alu_operand_a, alu_operand_b;
wire branch_alu_result;
